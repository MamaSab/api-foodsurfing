...
class PersonneController extends Controller
{
    ...
    {
        $results = app('db')->select("SELECT `idrepas`,`plat`,`nombre_minimum_personne`,`nombre_maximum_personne`,
        `dateRepas`,`lieu`,`description`,`personnes_idPersonnes`,`themes_idthemes`,
    ...
    {
        $results = app('db')->select("SELECT `repas_idrepas`,`ajout_idajout`,`dateAjout`, 
        repas.dateRepas as dr, repas.plat as plat, repasajout.personnes_idPersonnes as idp, 
        ajout.nomAjout as na, personnes.prenom as pp, personnes.nom as pn
        FROM `repasajout`
    ...
    public function store (Request $request)
     {
        $input = $request->all();
        $results = DB::insert('
        INSERT INTO personnes (nom, prenom, mot_de_passe, ville, age)
        VALUES ("'.$input["nom2"].'","'.$input["prenom"].'", "'.$input["mot_de_passe"].'","'.$input["ville"].'","'.$input["age"].'")');
